...
public class GlobalExceptionHandler {
  ...
  private ResponseEntity<BussinesExceptionEntity> handleDataIntegrity(
    HttpServletRequest request,
    DataIntegrityViolationException violationException
  ) {
    var bussiness = BussinesExceptionEntity
      .builder()
  ...
  private ResponseEntity<Object> response(
    BussinesException bussinesException,
    HttpServletRequest httpServletRequest
  ) {
    return new ResponseEntity<>(
      BussinesExceptionEntity
        .builder()
        .error(bussinesException.getMessage())
        .status(
          isNull(bussinesException.getStatusEnum())
            ? StatusError.UNKNOW_ERROR
            : bussinesException.getStatusEnum()
        )
        .arguments(bussinesException.getArgs())
        .time(dataAtual())
        .path(httpServletRequest.getServletPath())
        .build(),
      bussinesException.getStatusCode()
    );
  }
}
